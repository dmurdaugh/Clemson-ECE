...
void SelectionSort(list_t *list_ptr, list_node_t *M,list_node_t *N, int sort_type);
void IterativeSort(list_t *list_ptr,list_node_t *M,list_node_t*N, int sort_type);
void list_insertion_sort(list_t *list_ptr, int sort_type);
list_node_t *findMin(list_t *list_ptr,list_node_t *M,list_node_t *N,int sort_type);
list_node_t *merge(list_t *list_ptr,list_node_t *first, list_node_t *second, int sort_type);
list_node_t *mergeSort(list_t *list_ptr,list_node_t *head, int sort_type);
list_node_t *print(list_node_t *head);
list_node_t *split(list_node_t *head);
void merge2(list_node_t *first, list_node_t *second,int sort_type);
...
void list_destruct(list_t *list_ptr)
{
    ...
    rover_1=rover_2->next;
        ...
        //printf("working here...\n");
        //free(rover_2->data_ptr->routelist);
        //rover_2->data_ptr->routelist=NULL;
    ...
    while(rover_1->next!=NULL){
     ...
     //free(rover_2->data_ptr->routelist);
         ...
         rover_2->next=NULL;
         free(rover_2);
     //free(rover_1->data_ptr->routelist);
...
void list_insert(list_t *list_ptr, data_t *elem_ptr, int pos_index)
{
    ...
    if(list_ptr->current_list_size%10000==0){
        //printf("debugging\n");
        //list_debug_validate(list_ptr);
    }
}
...
data_t * list_remove(list_t *list_ptr, int pos_index)
{
    ...
    rover=last;
    list_ptr->current_list_size--;
    //printf("copy->src=%d copy->dest=%d\n",copy->src_ip_addr,copy->dest_ip_addr);
    /* fix the return value */
    return copy;
}
...
//Sort function that chooses which sort is appropriate for use.
/*void list_sort(list_t *list_ptr,int sort_type,int sort_order){*/
/*    //Insertion*/
/*    list_t *sort=list_ptr;*/
/*    if(sort_type==1){*/
/*        list_insertion_sort(list_ptr,sort_order);*/
/*    }*/
/*    //recursive selection*/
/*    else if(sort_type==2){*/
/*        SelectionSort(list_ptr,list_ptr->head,list_ptr->tail,sort_order);*/
/*    }*/
/*    //Iterative Selection*/
/*    else if(sort_type==3){*/
/*        IterativeSort(list_ptr,list_ptr->head,list_ptr->tail,sort_order);*/
/*    }*/
/*    //Merge Sort*/
/*    else if(sort_type==4){*/
/*        list_node_t *rover=NULL;*/
/*        int count=0;*/
/*        list_node_t *a=list_ptr->head;*/
/*        sort->head=mergeSort(list_ptr,a,sort_order);*/
/*        rover=sort->head;*/
...
/*        sort->tail=rover;*/
/*        if(sort_order==1){*/
/*            sort->list_sorted_state=LIST_SORTED_ASCENDING;*/
/*        }*/
/*        else{*/
/*            sort->list_sorted_state=LIST_SORTED_DESCENDING;*/
/*        }*/
/*        //list_debug_validate(sort);*/
/*        list_ptr=sort;*/
/*    }*/
/*    if(sort_type !=4){*/
/*        if(sort_order==1){*/
...
/*//Recursive Selection Sort; Uses algorithm based off of Standish's array implementation*/
/*void SelectionSort(list_t *list_ptr, list_node_t *M,list_node_t *N,int sort_type){*/
/*    list_node_t *minposition;*/
/*    data_t *temp;*/
/*    if(M != N){*/
/*        minposition=findMin(list_ptr,M,N,sort_type);*/
/*        temp=M->data_ptr;*/
/*        //printf("minimum = %d\n",minposition->data_ptr->src_ip_addr);*/
/*        M->data_ptr=minposition->data_ptr;*/
/*        minposition->data_ptr=temp;*/
/*        SelectionSort(list_ptr,M->next,N,sort_type);*/
/*    }*/
/*    list_node_t *rover=list_ptr->head;*/
/*    while(rover!=NULL){*/
/*        rover=rover->next;*/
...
/*        */
/*}*/
/*//Iterative Selection Sort: Uses algortihm based off of Standish's array implementation*/
/*void IterativeSort(list_t *list_ptr,list_node_t *M,list_node_t*N,int sort_type){*/
/*    list_node_t *min=M;*/
/*    while(M!=N){*/
/*        min=findMin(list_ptr,M,N,sort_type);      */
/*        data_t *temp=M->data_ptr;*/
/*        M->data_ptr=min->data_ptr;*/
/*        min->data_ptr=temp;*/
/*        M=M->next;*/
/*    }*/
/*}*/

//Insertion Sort; Takes from unsorted list and uses list_insert_sorted to 
// input into new list;
void list_insertion_sort(list_t *list_ptr,int sort_type){
    list_t *nlist;
    data_t *node;
    nlist=list_construct(NULL,NULL);
    nlist->comp_proc=list_ptr->comp_proc;
    if(sort_type==1){
        nlist->list_sorted_state=LIST_SORTED_ASCENDING;
    }
    else if(sort_type==2){
        nlist->list_sorted_state=LIST_SORTED_DESCENDING;
    }
    while(list_ptr->current_list_size >0){
        //node=list_ptr->head->data_ptr;
        node=list_remove(list_ptr,0);
        list_insert_sorted(nlist,node); 
    }
    list_ptr->head=nlist->head;
    list_ptr->tail=nlist->tail;
    list_ptr->current_list_size=nlist->current_list_size;
    list_ptr->list_sorted_state=nlist->list_sorted_state;
    free(nlist);
}

//Findmin function based off of Standish's example
list_node_t *findMin(list_t *list_ptr,list_node_t *M,list_node_t *N,int sort_type){
    list_node_t *i=M;
    list_node_t *j=M->next;
    int compare=0;
    while(j!=NULL){
        compare=(list_ptr->comp_proc)(j->data_ptr,i->data_ptr);
        if(sort_type==2){
            compare=compare*-1;
        }
        if(compare==1){
...
//
//
// merge subfunction that combines the two seperate list back together
// BUG: Breaks after too many calls, function input corruption
//list_node_t *merge(list_t *list_ptr,list_node_t *first, list_node_t *second,int sort_type){
/*    int count=0;*/
/*    int compare=0;*/
/*    */
...
/*        if(!first){*/
/*          if(count==0){*/
/*            list_insert(list_ptr,second->data_ptr,LISTPOS_HEAD);*/
/*          }*/
/*          list_insert(list_ptr,second->data_ptr,LISTPOS_TAIL);*/
/*          second=second->next;*/
/*        }*/
/*        if(!second){*/
/*          if(count==0){*/
/*            list_insert(list_ptr,first->data_ptr,LISTPOS_HEAD);*/
/*          }*/
/*          list_insert(list_ptr,first->data_ptr,LISTPOS_TAIL);*/
/*          first=first->next;*/
/*        }*/
/*        data_t *record_a, *record_b;*/
/*        record_a=first->data_ptr;*/
/*        record_b=second->data_ptr;*/
/*        compare=(list_ptr->comp_proc)(record_a,record_b);*/
...
/*        count++;*/
/*    }*/
/*    printf("head %d",list_ptr->head->data_ptr->src_ip_addr);*/
...
/*    head=mergeSort(list_ptr,head,sort_type);*/
/*    second= mergeSort(list_ptr,second,sort_type);*/
/*    */
/*    return merge(list_ptr,head,second,sort_type);*/
/*}*/

//
//
//Splits the list into a half with the odd entry going into the first list
list_node_t *split(list_node_t *head){
...
//
//
//print the list but within the list.c for debugging purposes
/*list_node_t *print(list_node_t *head)*/
/*{*/
/*    printf("Forward Traversal using next poitner\n");*/
/*    while (head)*/
/*    {*/
/*        printf("%d\t ",head->data_ptr->src_ip_addr);*/
/*        head = head->next;*/
/*    }*/
